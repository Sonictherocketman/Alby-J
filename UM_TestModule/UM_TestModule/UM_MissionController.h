...
const struct Vector MAX_MISSION_PARAMETER_TRAJECTORY = {1, 60, 8, 0};   //degrees from straight out the nose
const float MAX_V_CRUISE_FACTOR = 1.1;
const float MIN_V_CRUISE_FACTOR = 0.9;
const float AVERAGE_CRUISE_VELOCITY = 5;                                 //meters per second
...
public:
    ...
    //Getters and Setters.
    UM_Trajectory*& getOldTrajectory();
	void setOldTrajectory(UM_Trajectory*& oldTrajectory);
    UM_SensorDataCollection*& getSensorData();
	void setSensorData(UM_SensorDataCollection*& sensorData);
    UM_Trajectory*& getTrajectory();
	void setTrajectory(UM_Trajectory*& trajectory);
	UM_Actions* getWingActions();
	void setWingActions(UM_Actions* wingActions);
