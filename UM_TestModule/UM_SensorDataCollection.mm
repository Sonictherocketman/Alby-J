...
void UM_SensorDataCollection::refresh(float Z, float Vel, float Ax, float Ay, float Az, float T1, float T2, float T3,  float Gx, float Gy, float Gz)
{
	...
	if (this->old_Z == this->Z)	{this->newData = NO;}

	if (this->old_Vel == this->Vel)	{this->newData = NO;}

	if (this->old_Ax == this->Ax)	{this->newData = NO;}
	if (this->old_Ay == this->Ay)	{this->newData = NO;}
	if (this->old_Az == this->Az)	{this->newData = NO;}

	if (this->old_T1 == this->T1)	{this->newData = NO;}
	if (this->old_T2 == this->T2)	{this->newData = NO;}
	if (this->old_T3 == this->T3)	{this->newData = NO;}

	if (this->old_Gx == this->Gx)	{this->newData = NO;}
	if (this->old_Gy == this->Gy)	{this->newData = NO;}
	if (this->old_Gz == this->Gz)	{this->newData = NO;}
}
